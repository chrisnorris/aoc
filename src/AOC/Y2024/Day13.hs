...
bs (x,y) (as, bs) ba bb =
      let (ex, ey) = (x + as * (dx ba), y+ as * (dy ba)) in
                   (ex+ bs * (dx bb), ey+ bs * (dy bb))
...
getPrizes Prize{..} = let r = [ (a*3+b) | a <- [0..10000000000000], b <- [0..10000000000000], let (px, py) = bs (0,0) (a,b) buttonA buttonB , px == (xp prizeAm), py==(yp prizeAm)] in
                           if null r then 0 else minimum r
