...
main_pt1 = do
  input' <-
    ((\s -> getStartEnd <$> [s, reverse s]) <$>) . lines
...
loop f acc s@(l : ls) =
  if isDigit l
    then loop f (acc <> [l]) ls
    else case switchTodigits s (f <$> digitsAsWords) of
      (Nothing, _) -> loop f acc ls
      (Just p, b) -> loop f (acc <> [intToDigit p]) b

switchTodigits str daw =
  let m = (\w -> str ^? prefixed w) <$> daw
   in case findIndexOf traversed isJust m of
