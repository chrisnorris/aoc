...
getDigplans = do 
     ...
     let (Right plans) = sequence $ parse digPlanParser "" <$> al
     return $ 
       A.listArray ((-500, -500), (900, 900)) (repeat 0) A.//
         ((, 1) <$> snd (maxMin plans))
...
width line =
  ...
  let fandL = f $ reverse $ f (imap (,) line)
      ((s,_), (e,_)) = (last fandL, head fandL) in
  1 + e - s
 where f = dropWhile ((/= '#') . snd)
...
myf (endPt@(y, x), acc) =
  let p = (\case
            ...
            ('L', st) -> [(y,x-s) | s <- [0..st] ]
            ('U', st) -> [(y+s,x) | s <- [0..st] ]
            ('D', st) -> [(y-s,x) | s <- [0..st] ]) in
  (\v -> (last v, acc ++ v)) . p . asDirectionSteps

printAr a =
    let ((lx,ly), (ux, uy)) = A.bounds a
        xx = chunksOf (ux-lx+1) (A.elems $ (\x -> if x == 0 then ' ' else '#' ) <$> a) in
    forM_ xx print 

ar2map a =
    let ((lx,ly), (ux, uy)) = A.bounds a in
    chunksOf (ux-lx+1) (A.elems $ (\x -> if x ==0 then ' ' else '#' ) <$> a)
