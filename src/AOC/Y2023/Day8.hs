...
main_pt1 = do
 ...
 where  loop _ n "ZZZ" _ = n
        loop (i:is) n e m =
              let r = let result = fromJust $ M.lookup e m in if i == 'L' then head result else result !! 1 in
              loop is (n+1) r m
...
main_pt2 = do
  ...
  let (Right tuples) = sequence $ parse tuplesParser "" <$> drop 2 al
      m = M.fromList tuples
      starts = filter ( (=='A')  .  head . reverse ) (M.keys m)
  return $ loop (concat $ repeat $ head al) 0 starts (M.fromList tuples)

 where  loop _ n l _ | (finished l) = n
        loop (i:is) n l m =
