...
main_pt1 = do
  ...
  let halfway = midpoint <$> codes
      items = uncurry intersection . toSetTuple <$> zipWith splitAt halfway codes
...
priority c = 
  fromMaybe (error "missing") (Map.lookup c priorities)
...
intersectAll l = let (a,b) = bimap (fromList . head) (fmap fromList) (splitAt 1 l) in
     Prelude.foldl intersection a b
