...
main4 = do
  ...
  where
    play [] board = ([(True, 0, 0)], 0)
    play (round : games) boards =
      let thisRound = playANumber boards round
       in if fst3 $ head $ theresAWinner thisRound
            then (theresAWinner thisRound, round)
...
theresAWinner round =
  let winners =
        [ let board = getNthBoard round n
              ...
              unmarkeds = fst <$> filter ((== 0) . snd) (concat board)
           in (elem 5 $ sum . map snd <$> (ver <> hor <> [diag]), n, sum unmarkeds)
          | n <- [0 .. 99]
        ]
   in if True `elem` (fst3 <$> winners) then filter ((== True) . fst3) winners else [(False, 0, 0)]
