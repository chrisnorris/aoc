...
main3ii = -- 1184 2256
   readInstr >>= lifeSupportRating '0' '1'
   >>
   readInstr >>= lifeSupportRating '1' '0'

lifeSupportRating c d ins =
   foldM
     (\acc i ->
      let (x, zs) = bitat acc i in
      if x >= zs then debug (filterWithAtN acc i c) >> return (filterWithAtN acc i c)
      else debug (filterWithAtN acc i d) >> return (filterWithAtN acc i d)) ins [0..11]
