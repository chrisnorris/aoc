...
import           Library(inp21Str, Source(..))
...
main11 source = do
  ...
  let adjs = foldl (\m (Right (k1, k2)) ->
             let m1 = update k1 k2 m in update k2 k1 m1) Map.empty a  
   
  let paths adjs acc orig = let c = (last orig) in
        case Map.lookup c adjs of
      ...
      updtae mp vs k = 
        ...
        foldl (\m v -> 
          let idx = firstOcc (m ^. at v . _Just) k in
          (at v . _Just) . (ix (idx-1) . _2) .~ 1 $ m ) mp ( fst <$> vs)
  ...
  where getInput source = inp21Str ("d12.input" <> show source)
        update k1 k2 m = 
...
parseLink = do
  let pathName =
              Start <$> Parsec.try (string "start")
          <|> Caps  <$> Parsec.try (many1 (oneOf ['A' .. 'Z']))
          <|> Lower <$> Parsec.try (many1 (oneOf ['a' .. 'z']))
          <|> End   <$> Parsec.try (string "end")
  s <- pathName
  char '-'
  s2 <- pathName
  return (s, s2)
